document.addEventListener("DOMContentLoaded", () => {
  ...
  function buildQuiz() {
    ...
    quizQuestions = sourceQuestions.map(q => ({
      prompt: q.q,
  ...
  function reviewWrongAnswers() {
    ...
    quizQuestions.forEach((q, i) => {
      const selectedIndex = selectedAnswers[i];
      const selectedText =
        selectedIndex === null ? "No answer selected" : q.options[selectedIndex];
      const isCorrect =
        selectedIndex !== null && q.options[selectedIndex] === q.answerText;
    ...
    if (wrongCount === 0) {
      reviewArea.innerHTML =
        `<div class="review-item"><p class="correct">Perfect score. No wrong answers to review.</p></div>`;
    }
